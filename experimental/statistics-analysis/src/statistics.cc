...
void dump_disk(std::vector<std::vector<cv::Point3f> > inputpoints,
               grid_params grid_description, motion_type motion, float angle,
               int num_images, float distance, camera_params intrinsics,
               cv::Point3f starting_point,
               std::vector<camera_frame> &cam_frames,
               std::string dump_directory, bool dump_images) {
  ...
  // grid specs
  infofile << grid_description.gridx << "\t" << grid_description.gridy << "\n";
  // angle
  infofile << angle << "\n";
  // motion direction
  infofile << motion << "\n";
  ...
  infofile << distance << "\n";
  // intrinsics
  infofile << intrinsics.f << "\t" << intrinsics.cx << "\t" << intrinsics.cy
           << "\n";
  // starting pt
  ...
  if (dump_images) {
    ...
    for (int i = 0; i < cam_frames.size(); i++) {
      cv::imwrite(dump_directory + "/image_" + std::to_string(i) + ".png",
                  cam_frames[i].image);
    }
  }
};
