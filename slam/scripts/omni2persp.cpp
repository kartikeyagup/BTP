...
public:
	...
	//CVec2d& operator = (const CVec2d& rhs) // compound assignment (does not need to be a member,
...
public:
	...
	//CVec2f& operator = (const CVec2f& rhs) // compound assignment (does not need to be a member,
	//{                           // but often is, to modify the private members)
	//							/* addition of rhs to *this takes place here */
	//	return 
	//	
...
//		z = 0.0;
//	}
//	CVec3f(double p, double q, double r)
//	{
//		x = p;
//		y = q;
//		z = r;
...
//
//	}
//	double operator*(const CVec3f& rhs) // compound assignment (does not need to be a member,
//	{                           // but often is, to modify the private members)
//								/* addition of rhs to *this takes place here */
//		double val;
//		val = this->x*rhs.x + this->y*rhs.y + this->z*rhs.z;
...
public:
	...
	/*CVec2f& operator ()(int x, int y) 
	{
		return this->arr[x][y];
	}*/
};

//double amax(double p, double q, double r, double s)
//{
		...
		const int win, const int hin, const int wout, const bool crop)
	{
		...
		for (int y = 0; y < hout; y++)
		{
			for (int x = 0; x < wout; x++)
			{
				...
				//+wright*x / (wout - 1) + wup*y / (hout - 1);
				//std::cout << "Wbase\n";
				//std::cout << wbase.x << "\t" << wbase.y << "\n";
				//std::cout << (wright*x).x << "\t" << (wright*x).y << "\n";
				...
				//std::cout << y << "\t" << x << " : " << warp.arr[y][x].y << "\t" << warp.arr[y][x].x << "\n";
				//= CVec2f(f.x, f.y);
			}
		}
...
int main()
{
	...
	while (1) {
		...
		// {
		// 	/*if (framecounter >= 42)
		// 	printf("failed to grab waiting\n");*/
		// 	// if (grabber_counter < 25)
		// 	// {
		// 	// 	cv::waitKey(1000);
		// 	// 	grabber_counter++;
		// 	// 	continue;
