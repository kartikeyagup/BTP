...
bool GetEssentialRT(corr& corres, TwoViewInfo& twoview_info,
  ...
  return VerifyTwoViewMatches(options, intrinsics1, intrinsics2,
...
bool VerifyTwoViewMatches(
  ...
  if (!EstimateTwoViewInfo(options.estimate_twoview_info_options, intrinsics1,
                           intrinsics2, correspondences, twoview_info,
                           inlier_indices)) {
    return false;
  }
...
void ShowCorres(std::string FLAGS_dirname, corr compressed) {
  cv::Mat im1 = cv::imread(FLAGS_dirname + "/img_" +
                           std::to_string(compressed.frame_1) + ".jpg");
  cv::Mat im2 = cv::imread(FLAGS_dirname + "/img_" +
                           std::to_string(compressed.frame_2) + ".jpg");
  cv::Size sz1 = im1.size();
  cv::resize(im1, im1, cv::Size(sz1.width / 2, sz1.height / 2));
  cv::resize(im2, im2, cv::Size(sz1.width / 2, sz1.height / 2));
  sz1 = im1.size();
  system(("mkdir " + FLAGS_dirname + "/corr_" +
          std::to_string(compressed.frame_1) + "_" +
          std::to_string(compressed.frame_2))
             .c_str());
  for (int i = 0; i < compressed.p1.size() / 10; i++) {
    ...
    for (int j = 10 * i; j < 10 * (i + 1); j++) {
      cv::circle(im3,
                 cv::Point2f(compressed.p1[j].x / 2, compressed.p1[j].y / 2), 3,
                 cv::Scalar(255, 0, 0), -1);
      cv::circle(im3, cv::Point2f(compressed.p2[j].x / 2 + sz1.width,
                                  compressed.p2[j].y / 2),
                 3, cv::Scalar(255, 0, 0), -1);
      cv::line(im3, cv::Point2f(compressed.p1[j].x / 2, compressed.p1[j].y / 2),
               cv::Point2f(compressed.p2[j].x / 2 + sz1.width,
                           compressed.p2[j].y / 2),
               cv::Scalar(0, 0, 255));
    }
    cv::imwrite(FLAGS_dirname + "/corr_" + std::to_string(compressed.frame_1) +
                    "_" + std::to_string(compressed.frame_2) + "/" +
                    std::to_string(i) + ".jpg",
                im3);
  }
}
